  ...
  AddProductToCart(id: Number, soluong?: number) {
    ...
    this.productService.getSingleProduct(id).subscribe((prod:any) => {
      ...
      if (this.cartDataServer.data[0].product === undefined) {
        ...
        localStorage.setItem('cart', JSON.stringify(this.cartDataClient));
        this.cartDataObs$.next({...this.cartDataServer});
        this.toast.success(`${prod.tensanpham} added to the cart.`, "Product Added", { //thêm vào giỏ hàng sản phẩm được thêm
          timeOut: 1500,
          progressBar: true,
      ...
      // Cart is not empty
        ...
        let index = this.cartDataServer.data.findIndex(p => p.product.masp === prod.masp);
        ...
        if (index !== -1) {
          ...
          if (soluong !== undefined) {
            // @ts-ignore
            this.cartDataServer.data[index].numInCart = this.cartDataServer.data[index].numInCart + soluong < prod.soluong ? Number(soluong) + Number(this.cartDataServer.data[index].numInCart) : prod.soluong;
          } else {
            // @ts-ignore
          ...
          this.toast.info(`${prod.tensanpham} quantity updated in the cart.`, "Product Updated", {
            timeOut: 1500,
            progressBar: true,
            progressAnimation: 'increasing',
            positionClass: 'toast-top-right'
        ...
        // 2. If chosen product is not in cart array
          ...
          this.toast.success(`${prod.tensanpham} added to the cart.`, "Product Added", {
            timeOut: 1500,
            progressBar: true,
            progressAnimation: 'increasing',
            positionClass: 'toast-top-right'
